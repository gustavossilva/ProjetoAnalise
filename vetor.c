...
void escreva_vetor_int(int * v, int n, char * arq){
  int i;
  FILE* fd = NULL;
  
  fd = fopen(arq, "w");
  CONFIRME(fd!= NULL, "escreva_vetor_int: fopen falhou\n");
  
  // Na primeira linha está o número de elementos
  fprintf(fd, "%d\n", n); 
  for(i=0; i<n; i++)
    fprintf(fd, "%d\n", v[i]);
  fclose(fd);
...
void escreva_vetor_double(double * v, int n, char * arq){
  int i;
  FILE* fd = NULL;
  
  fd = fopen(arq, "w");
  CONFIRME(fd!= NULL, "escreva_vetor_double: fopen falhou\n");
